...
zf_values = list(range(60000, 80000, 10))  # Continue the list in steps of 2000 up to 200000
...
for i, current_zf in enumerate(zf_values):
    ...
    # Resize the cropped region back to the original image display size (for normalization)
    final_display_image_resized = cv2.resize(cropped_intensity_cpu, 
                                     (original_size_x, original_size_y), 
                                     interpolation=cv2.INTER_LINEAR)

    # Normalize intensity for display consistency (0-1 range)
    display_image_normalized = (final_display_image_resized - final_display_image_resized.min()) / \
                               (final_display_image_resized.max() - final_display_image_resized.min() + 1e-9) # Add epsilon to avoid div by zero

    # --- Calculate Coefficient of Variation (CV) on the NORMALIZED image ---
    # Convert the normalized image (which is on CPU as numpy array) to CuPy for CV calculation
    image_for_cv_gpu = cp.asarray(display_image_normalized, dtype=cp.float32) 
...
plt.title('Coefficient of Variation vs. Focus Distance (on Normalized Images)')
plt.grid(True)

# Save the plot (optional)
plt.savefig('cv_vs_zf_plot_normalized.png')
